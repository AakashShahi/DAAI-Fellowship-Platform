...
export default function SubmissionReviewView({ listPath, heading = 'Review submission' }) {
  ...
  if (isLoading) {
    return (
      <section>
        <p className="text-sm font-medium text-[#475569]">Loading submission…</p>
      </section>
    )
  }
  ...
  if (error && !detail) {
    return (
      <section>
        <Link className="text-sm font-bold text-[#4f46e5]" to={listPath}>
          ← Back to submissions
        </Link>
  ...
  return (
    <section>
      <div className="mb-6">
        ...
        <p className="mt-3 text-xs font-black uppercase tracking-wide text-[#4f46e5]">
          Submissions
        </p>
        <h1 className="mt-2 text-3xl font-black text-[#0f172a] lg:text-4xl">{heading}</h1>
      ...
      <div className="mb-8 grid gap-6 lg:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-lg font-black text-[#0f172a]">Assignment</h2>
          <p className="mt-2 text-sm font-medium text-[#475569]">{assignment.description}</p>
          <div className="mt-4 whitespace-pre-wrap text-sm text-[#0f172a]">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-lg font-black text-[#0f172a]">Fellow submission</h2>
          ...
          {submission.submissionText ? (
            <div className="mt-4 max-h-64 overflow-auto whitespace-pre-wrap rounded-md bg-[#f8fafc] p-3 text-sm text-[#0f172a]">
              {submission.submissionText}
            </div>
          ) : (
            <p className="mt-4 text-sm text-[#475569]">No written response.</p>
          )}
        </div>
      </div>
      ...
      >
        <h2 className="text-lg font-black text-[#0f172a]">Your review</h2>
        ...
        <button
          type="submit"
          className="mt-4 rounded-md bg-[#4f46e5] px-4 py-2 text-sm font-black text-white disabled:opacity-60"
          disabled={isSaving}
        >
