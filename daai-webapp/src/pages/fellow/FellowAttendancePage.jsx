...
export default function FellowAttendancePage() {
  ...
  return (
    <section>
      ...
      {isLoading ? <p className="mt-5 rounded-lg border border-orange-100 bg-white p-5 text-sm font-bold">Loading attendance...</p> : (
        <div className="mt-5 overflow-hidden rounded-lg border border-orange-100 bg-white shadow-sm">
              ...
              <tbody className="divide-y divide-orange-100">
                {rows.map((row) => (
                  <tr key={row.sessionId}>
                    <td className="px-4 py-4 font-black text-[#24140e]">{row.sessionTitle}</td>
                    <td className="px-4 py-4 text-[#6f5f57]">{new Date(row.sessionDate).toLocaleDateString()}</td>
                    <td className="px-4 py-4"><AttendanceStatusBadge status={row.status} /></td>
                    <td className="px-4 py-4 text-[#6f5f57]">{row.remarks || '—'}</td>
                    <td className="px-4 py-4 text-[#6f5f57]">{row.markedAt ? new Date(row.markedAt).toLocaleString() : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
