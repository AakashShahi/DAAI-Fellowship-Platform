...
export default function ModuleDetailPage() {
  ...
  if (isLoading) return <p className="rounded-lg border border-orange-100 bg-white p-5 text-sm font-bold">Loading module...</p>
    ...
    <section>
      <div className="mb-6 rounded-lg border border-orange-100 bg-white p-6 shadow-[0_18px_45px_-28px_rgba(112,55,23,0.35)]">
        ...
        <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h1 className="text-3xl font-black text-[#24140e]">{module.title}</h1>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <Link className="text-sm font-black text-[#f26322]" to={`/admin/modules/${module.id}/edit`}>
            Edit module
          </Link>
          <button
            type="button"
            className="text-sm font-black text-[#f26322]"
      ...
      <div className="grid gap-3">
        {module.lessons.length === 0 ? (
          <p className="rounded-lg border border-orange-100 bg-white p-5 text-sm font-bold text-[#6f5f57]">No lessons yet.</p>
        ) : (
          module.lessons.map((lesson) => (
            <article key={lesson.id} className="rounded-lg border border-orange-100 bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <p className="text-xs font-black uppercase tracking-wide text-[#f26322]">
                    Lesson {lesson.order} · {lesson.isPublished ? 'Published' : 'Draft'} · {lesson.estimatedDurationMinutes} min
                  </p>
                  <h2 className="mt-1 text-xl font-black text-[#24140e]">{lesson.title}</h2>
                  <p className="mt-2 text-sm font-medium text-[#6f5f57]">{lesson.description}</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <button type="button" className="rounded-md border border-orange-100 px-3 py-2 text-sm font-black text-[#f26322]" onClick={() => setEditingLesson(lesson)}>
                    Edit lesson
                  </button>
