...
export default function AssignmentSubmissionsPage() {
  ...
  if (!assignment) return <p className="rounded-lg border border-slate-200 bg-white p-5 text-sm font-bold">Loading submissions...</p>

  return (
    <section>
      <div className="mb-6 rounded-lg border border-slate-200 bg-white p-6 shadow-[0_18px_45px_-28px_rgba(15,23,42,0.35)]">
        <p className="text-xs font-black uppercase tracking-wide text-[#4f46e5]">Submission Review</p>
        <h1 className="mt-2 text-3xl font-black text-[#0f172a]">{assignment.title}</h1>
      </div>
      {error ? <p className="mb-5 rounded-lg border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-700">{error}</p> : null}
      <div className="grid gap-4">
        {submissions.length === 0 ? <p className="rounded-lg border border-slate-200 bg-white p-5 text-sm font-bold">No submissions yet.</p> : submissions.map((submission) => (
          <article key={submission.id} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <h2 className="font-black text-[#0f172a]">{submission.fellowName}</h2>
                <p className="mt-1 text-sm font-medium text-[#475569]">{submission.fellowEmail}</p>
              </div>
              <SubmissionStatusBadge status={submission.status} />
            </div>
            <p className="mt-4 whitespace-pre-wrap text-sm text-[#0f172a]">{submission.submissionText}</p>
            <div className="mt-3 grid gap-2">
              {submission.submissionLinks.map((link) => <a key={link.url} className="text-sm font-black text-[#4f46e5]" href={link.url} target="_blank" rel="noreferrer">{link.title} · {link.type}</a>)}
            </div>
            <p className="mt-3 text-sm font-bold text-[#475569]">Submitted {new Date(submission.submittedAt).toLocaleString()} · Marks {submission.marksObtained ?? '—'}</p>
