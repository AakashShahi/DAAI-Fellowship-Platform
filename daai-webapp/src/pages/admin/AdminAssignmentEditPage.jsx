...
export default function AdminAssignmentEditPage() {
  ...
  if (!assignment) return <p className="rounded-lg border border-orange-100 bg-white p-5 text-sm font-bold">Loading assignment...</p>

  return (
    <section>
      <div className="rounded-lg border border-orange-100 bg-white p-6 shadow-[0_18px_45px_-28px_rgba(112,55,23,0.35)]">
        <p className="text-xs font-black uppercase tracking-wide text-[#f26322]">Assignment Detail</p>
        ...
        <div className="mt-5 grid gap-3 text-sm font-bold text-[#6f5f57] md:grid-cols-4">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <Link className="rounded-md bg-[#f26322] px-4 py-2 text-sm font-black text-white" to={`/admin/assignments/${assignment.id}/submissions`}>View Submissions</Link>
          <Link className="rounded-md border border-orange-100 px-4 py-2 text-sm font-black text-[#f26322]" to={`/admin/assignments/${assignment.id}/edit`}>Edit Assignment</Link>
        </div>
      </div>
    </section>
  )
